...
|---------|---------------|----------|------------|
| acme | `companies/acme/knowledge/` | Own repo (has `.git/`) | ~40 files across 8 subdirs |
| widgets | `companies/widgets/knowledge/` | Own repo (has `.git/`) | ~20 files across 7 subdirs |
| designco | `companies/designco/knowledge/` | Own repo (has `.git/`) | ~12 files across 3 subdirs |
| personal | `companies/personal/knowledge/` | Own repo (has `.git/`) | ~8 files |
| side-project | N/A | No knowledge dir | 0 |
...
pub struct KnowledgeRepo {
    ...
    pub scope: String,             // "hq" or company ID ("acme", etc.)
...
3. **Nested symlinks:** Some knowledge repos contain internal symlinks (e.g., Acme Corp CDP knowledge has `audit/` and `flow-migration/` symlinks). Desktop should resolve these recursively.
...
| `companies/acme/knowledge/INDEX.md` | Yes | Lists LR knowledge files/dirs |
| `companies/widgets/knowledge/INDEX.md` | Yes | Lists Widgets Inc knowledge files/dirs |
| `companies/designco/knowledge/INDEX.md` | Yes | Lists Design Co knowledge files/dirs |
...
- Name (e.g., "Ralph", "HQ Core", "Acme Corp")
...
acme:
  ...
  qmd_collections: [acme, {repo}]

widgets:
  knowledge: companies/widgets/knowledge/
  qmd_collections: [widgets]

designco:
  knowledge: companies/designco/knowledge/
  qmd_collections: [designco]
...
1. **Company filter in UI**: When user selects a company context (e.g., "Acme Corp"), the knowledge browser should:
...
│   └── Private
│       └── Linear
├── Company: Acme Corp (when LR context active)
│   ├── Architecture
│   ├── Database Schema
│   ├── Infrastructure
│   ├── {repo}/
...
│   ├── GTM/
│   └── ...
├── Company: Widgets Inc (when Widgets Inc context active)
│   ├── Brand Guidelines
│   ├── Master Narrative
│   ├── CRO/
│   └── ...
...
| `acme` | LR knowledge | `**/*.md` | 121 | 1 | LR company knowledge |
| `widgets` | Widgets Inc knowledge | `**/*.md` | 87 | 1 | Widgets Inc company knowledge |
| `designco` | Design Co knowledge | `**/*.md` | 15 | 1 | Design Co company knowledge |

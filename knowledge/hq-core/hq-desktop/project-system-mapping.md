...
| `repoPath` | string | Target repo relative to HQ (e.g. `repos/private/{repo}`) |
...
| E2E manifest | `npm run generate-manifest && git diff --quiet tests/e2e/manifest.json` | Widgets Inc-site only |
| Coverage | `npm run check-coverage` | Widgets Inc-site only |

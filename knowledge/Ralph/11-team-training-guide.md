...
**Decision: Ralph for features, traditional approach for bugs**
...
- New beta version released with citation feature (developed by Developer)
...
2. Set up a sandbox environment (Developer's approach -- safe place to experiment)
...
1. **Start small, one feature at a time** -- Developer proved this with auth. Don't try to Ralph everything on day 1.

2. **Clean codebase = faster Ralph** -- Messy code slows everything 10x. Cleanup is investment, not waste.

3. **Use a sandbox to learn** -- Developer's approach: safe sandbox first to learn principles and set up personal workflow, then apply to real work.

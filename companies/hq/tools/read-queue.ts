 ...
 *
 * Usage:
 *   npx tsx companies/hq/tools/read-queue.ts [-c <company-slug>]
 *   npx tsx companies/hq/tools/read-queue.ts -c hq --status all
 *   npx tsx companies/hq/tools/read-queue.ts --json
 *   npx tsx companies/hq/tools/read-queue.ts --n 10
 */
...
function getCompanySlug(argv: string[]): string {
  ...
  return idx !== -1 && argv[idx + 1] ? argv[idx + 1] : "hq";
}

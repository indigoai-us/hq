...
Options:
  -c, --company SLUG     Company slug (default: ghq)
...
Examples:
  $(basename "$0") show ghq-5
  $(basename "$0") list --status open
  $(basename "$0") -c acme create --title "Fix login bug"
  $(basename "$0") close ghq-5.1
  $(basename "$0") children ghq-5
  $(basename "$0") comments add ghq-5 "PR ready: https://github.com/..."
EOF
  exit 0
}

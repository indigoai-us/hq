...
| Collection badge | Extracted from `result.file` prefix | Small pill badge: `text-xs`, e.g., "HQ", "LiveRecover" |
...
| "LiveRecover" | Show HQ public + HQ private + LiveRecover knowledge only |
...
**For the file tree:** Not needed in v1. Even the largest knowledge base (LiveRecover, ~40 files) produces a tree of ~100 nodes when fully expanded. Standard DOM rendering handles this fine.
...
interface KnowledgeBase {
  name: string              // "Ralph", "hq-core", "liverecover"
  displayName: string       // Title-cased: "Ralph", "HQ Core", "LiveRecover"
...
interface QmdCollection {
  name: string              // "hq", "{product}", "liverecover", etc.
  fileCount: number

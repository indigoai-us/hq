...
```yaml
- id: analyst-worker
  path: workers/private/analyst-worker/
  type: ResearchWorker
  visibility: private        # NOT parsed
  company: acme        # NOT parsed
  status: active
  description: "..."
```
...
acme:
  ...
  workers: [cfo-worker, analyst-worker, {repo}-deploy]
...
{ id: 'cfo-worker', type: 'FinanceWorker', team: 'finance' }
{ id: 'cmo-worker', type: 'MarketingWorker', team: 'marketing' }
...
**Issues:**
- Lists `band-tbd` which doesn't exist (actual: side-project)
- Missing `side-project` company
...
{ name: 'widgets-billion-dollar-website', state: 'IN_PROGRESS', prdPath: '...README.md' }
```

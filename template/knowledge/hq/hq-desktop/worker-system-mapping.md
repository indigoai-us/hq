...
| ContentWorker | 5 | content-brand, content-sales, {company-2}-brand-writer |
| ResearchWorker | 2 | {company}-analyst, pr-monitor |
| SocialWorker | 1 | x-{your-handle} |
...
integrations:
  x:
    client: workers/private/x-{your-handle}/src/x-client.ts
...
companies/manifest.yaml
  │
  ├─ {company-1}:
  │     workers: [cfo-{company}, {company}-analyst, {company}-deploy, infobip-admin, {co1}-qa, {company}-gtm]
  │
  ├─ {company-2}:
  │     workers: [cmo-{company}, {company-2}-brand-writer, {company-2}-copy-auditor]
  │
  ├─ {company-3}:
  │     workers: [cmo-{company}]
  │
  ├─ personal:
  │     workers: [x-{your-handle}, invoices]
  │

...
| Collection badge | Extracted from `result.file` prefix | Small pill badge: `text-xs`, e.g., "HQ", "{Company-1}" |
...
| "{Company-1}" | Show HQ public + HQ private + {Company-1} knowledge only |
| "{Company-2}" | Show HQ public + HQ private + {Company-2} knowledge only |
...
**For the file tree:** Not needed in v1. Even the largest knowledge base ({Company-1}, ~40 files) produces a tree of ~100 nodes when fully expanded. Standard DOM rendering handles this fine.
...
interface KnowledgeBase {
  name: string              // "Ralph", "hq-core", "{company-1}"
  displayName: string       // Title-cased: "Ralph", "HQ Core", "{Company-1}"
...
interface QmdCollection {
  name: string              // "hq", "{product}", "{company-1}", etc.
  fileCount: number

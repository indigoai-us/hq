...
## Voyage Linear Integration

All VYG/Voyage/{Company} work tracked in Linear (workspace: `voyage`). Same hierarchy: Initiatives > Projects > Issues > Sub-issues.
...
1. **`/prd` → create Linear project + issues**: On {Company}/Voyage PRD creation, create Linear project (linked to best-fit initiative), create issue per story, store IDs in prd.json
...
5. **Initiative grouping**: Reuse existing (CDP Migration, VYG.ai, Build Faster, Deliver More Value, Capture TAM, Automate Internal Ops, Grow Wallet Share). Only create new for genuinely new strategic themes
...
10. **Cross-posting guard**: NEVER use another company's Linear creds for Voyage work. Validate `workspace: "voyage"` in config before any API call
...
`/check-linear-voyage` — interactive triage for Voyage workspace. Enforces company isolation.
...
**Company** (`companies/{co}/workers/`): {company} (6), {company}/PR (6), personal (3), {company} (2), indigo (1). Full list: `workers/registry.yaml`.
...
**Collections:** `hq` (all HQ), `vyg` (VYG monorepo), `{company}`, `indigo`, `personal` (per-company). Use `-c {collection}` to scope searches. When working on a specific company, prefer `-c {company}` to avoid cross-company results.
...
- Add `-c {collection}` to scope to a specific collection (e.g. `-c vyg`)
...
| Find code by concept | `qmd vsearch -c vyg` | "where auth middleware is defined" |
...
| Find files by path pattern | `Glob` with scoped `path:` | `Glob pattern="*.ts" path="repos/private/vyg/apps/"` |
...
### VYG (`repos/private/vyg`)
**MANDATORY before any PR (enforced by hook):**
1. `bun run test` — must pass
2. `bun check` — must pass (TypeScript)
3. `bun lint` — must pass

**Use `/vyg-pr` command** (in VYG repo's `.claude/commands/`) — direct `gh pr create` is blocked by PreToolUse hook.
...
**Full rules:** `repos/private/vyg/.claude/CLAUDE.md`

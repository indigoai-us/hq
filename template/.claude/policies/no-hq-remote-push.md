...
NEVER push HQ data to any remote repository. HQ is local-only. The `origin` remote (`hq-starter-kit`) is used only for PULLING upstream updates, not for pushing local state.

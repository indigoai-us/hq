...
{company} had `supabase/.temp/project-ref` pointing to `zgxfkfbarlhkfzlkijnf` but `.env.local` URL pointed to `wfzqpgjhxrekdfflbmxl`. Using the CLI key gave "Invalid API key" errors. The production project was Vercel-managed and required `vercel env pull --environment production` to get the correct key.

...
macOS is case-insensitive but `pwd` may return a different casing than the hardcoded path. Example: `pwd` returns `~/Documents/hq` but `HQ_ROOT` is set to `~/HQ` — string comparison fails silently.

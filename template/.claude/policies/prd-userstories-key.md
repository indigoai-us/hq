...
Session 2026-03-30: `/run-project {your-project}` failed at launch because the PRD used `"stories"` instead of `"userStories"`. Required manual fix before execution could begin. The script has 80+ jq expressions referencing `.userStories[]` — there is no fallback.

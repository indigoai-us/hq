...
Discovered during a website redesign project. Paper MCP's `write_html` and `create_artboard` tools operate on the same document state. Concurrent writes from parallel agents would produce non-deterministic artboard placement and potential node ID conflicts.

---
description: Research VYG codebase and generate PRD for {Company}/Voyage features
allowed-tools: Task, Read, Glob, Grep, Write, Bash, AskUserQuestion
argument-hint: [feature description] [optional: DEV-xxx]
visibility: public
---

# /vyg-prd — VYG Feature Planning & PRD Generation

Research-first PRD generator for the VYG monorepo ({Company}/Voyage). Produces a technical PLAN in the repo AND an orchestrator-compatible prd.json in HQ.
...
Gold standard for adding features to VYG:
...
If empty, ask: "What VYG feature do you want to plan?"

## Step 2: Research VYG Codebase

**This is the key step.** Research BEFORE asking questions.

**2a. Semantic search:**
```bash
qmd vsearch "<feature keywords>" -c vyg --json -n 15
```

**2b. Read relevant infra:**
- `repos/private/vyg/apps/function/infra/index.ts` — what's wired
- `repos/private/vyg/apps/function/infra/tables.ts` — existing DynamoDB tables
...
**2e. Git history:**
```bash
cd repos/private/vyg && git log --oneline -20 -- <relevant paths>
```
...
Write to `repos/private/vyg/PLAN-{name}.md`:
...
**Repo:** repos/private/vyg
**Branch:** {branchName}

## Technical Plan
See [PLAN-{name}.md](../../../repos/private/vyg/PLAN-{name}.md) for detailed implementation plan with key decisions, file paths, and code patterns.
...
5. Create one issue per story (team routing per CLAUDE.md Voyage table, default DEV)
...
Files:
  ...
  repos/private/vyg/PLAN-{name}.md                  (technical implementation plan)
...
- **HARD BLOCK: Do NOT implement** — ONLY create PLAN + PRD files. NEVER edit VYG source during /vyg-prd
- **STOP after creation** — run `/handoff`, end session. Execution requires fresh session
- **Company hardcoded** — always {Company}, always `repos/private/vyg`, always Voyage Linear

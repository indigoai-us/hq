...
#
# Profiles:
#   minimal - Critical safety + knowledge hooks (includes protect-core)
...
is_in_standard_profile() {
  case "$1" in
    block-hq-glob|block-hq-grep|warn-cross-company-settings|detect-secrets|consult-knowledge|report-issue-reminder|auto-checkpoint-trigger|auto-handoff-trigger|observe-patterns|block-inline-story-impl|screenshot-resize-trigger|auto-reindex|learn-reminder|capture-learnings|protect-core)
      return 0 ;;
    *) return 1 ;;
  esac
}

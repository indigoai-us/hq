...
`template/` is the **canonical source** for all HQ content shipped via `npx create-hq`. As of v10.2.0 (hq-consolidation), `hq-starter-kit` has been archived and is no longer updated. All changes go directly to `template/` in this repo.

> **`hq-starter-kit` is archived.** Do not sync from it. See [github.com/coreyepstein/hq-starter-kit](https://github.com/coreyepstein/hq-starter-kit) — final release v10.2.1 points here.

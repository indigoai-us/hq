...
CORS is restricted to the production domain (`example.com`) and the local dev server (`localhost:5173`).
...
4. **CORS restriction** — The S3 bucket and API only accept requests from `example.com` and `localhost:5173`, blocking cross-origin access from other domains.

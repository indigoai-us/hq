...
const bucket = new s3.Bucket(stack, "Uploads", {
  cors: [
    {
      allowedOrigins: ["https://hq.{company}ai.com", "http://localhost:5173"],
...
CORS is restricted to the production domain (`hq.{company}ai.com`) and the local dev server (`localhost:5173`).
...
new StaticSite(stack, "Web", {
  path: "apps/web",
  customDomain: "hq.{company}ai.com",
});
```
...
4. **CORS restriction** — The S3 bucket and API only accept requests from `hq.{company}ai.com` and `localhost:5173`, blocking cross-origin access from other domains.

...
│   ├── hq-cli/         # @{company}ai/hq-cli
│   └── hq-cloud/       # @{company}ai/hq-cloud
├── apps/
│   ├── web/            # PWA dashboard
...
| `packages/hq-cli` | `@{company}ai/hq-cli` | CLI for running commands, workers, and sessions |
| `packages/hq-cloud` | `@{company}ai/hq-cloud` | Cloud sync, auth, and remote storage client |

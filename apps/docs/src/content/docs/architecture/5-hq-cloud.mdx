---
title: hq-cloud
description: Deep dive into @indigoai/hq-cloud — the S3 sync engine that powers bidirectional file synchronization between a local HQ instance and {Product}AI cloud storage.
---

`@indigoai/hq-cloud` is the most complex package in the HQ monorepo. It handles authentication with {Product}AI, bidirectional file synchronization to S3, background daemon management, file change detection, and conflict resolution via a local journal. The CLI delegates all sync operations to this package.

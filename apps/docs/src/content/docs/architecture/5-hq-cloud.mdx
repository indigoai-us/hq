---
title: hq-cloud
description: Deep dive into @{company}ai/hq-cloud — the S3 sync engine that powers bidirectional file synchronization between a local HQ instance and {Product}AI cloud storage.
---

`@{company}ai/hq-cloud` is the most complex package in the HQ monorepo. It handles authentication with {Product}AI, bidirectional file synchronization to S3, background daemon management, file change detection, and conflict resolution via a local journal. The CLI delegates all sync operations to this package.
...
Authentication uses an OAuth flow with `hq.{company}ai.com`. The flow mints a refresh token locally, which is then exchanged for temporary AWS STS credentials on demand.

### OAuth Flow

```
┌──────────┐     1. Open browser      ┌───────────────────┐
│  hq CLI  │ ─────────────────────────▶│ hq.{company}ai.com/  │
│          │                           │ auth?callback=...  │
│          │                           └────────┬──────────┘
│          │                                    │
│          │     2. User signs in               │
...
2. The user's browser opens to `https://hq.{company}ai.com/auth?callback=http://localhost:19847/callback`.
3. After sign-in, {Product}AI redirects to the callback URL with query parameters: `token`, `refresh_token`, `user_id`, `bucket`, and `region`.

...
Authentication uses an OAuth flow with `example.com`. The flow mints a refresh token locally, which is then exchanged for temporary AWS STS credentials on demand.
...
2. The user's browser opens to `https://example.com/auth?callback=http://localhost:19847/callback`.

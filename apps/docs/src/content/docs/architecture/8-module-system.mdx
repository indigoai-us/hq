...
modules:
  ralph-methodology:
    ...
    repo: https://github.com/{company}ai-us/ralph-methodology
    branch: main
    path: docs
    strategy: link
    targets:
  ...
  knowledge-workers:
    ...
    repo: https://github.com/{company}ai-us/knowledge-workers
    strategy: link
    targets:
...
The repository is cloned into `repos/public/` or `repos/private/` following the standard convention. The module name is extracted from the GitHub URL by default (e.g., `https://github.com/{company}ai-us/ralph-methodology` becomes `ralph-methodology`).
...
**`parseRepoName()`** — Extracts a module name from a GitHub URL by taking the final path segment and stripping any `.git` suffix. For example, `https://github.com/{company}ai-us/knowledge-workers.git` becomes `knowledge-workers`.
...
hq modules add https://github.com/{company}ai-us/knowledge-design-styles \
  --strategy link \

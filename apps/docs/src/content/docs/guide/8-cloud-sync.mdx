...
Local HQ  ←→  Sync Daemon  ←→  S3 Bucket  ←→  PWA Dashboard
              (chokidar)       (AWS)          (hq.{company}ai.com)
```
...
This opens your browser for {Product}AI sign-up/sign-in. After authentication:
...
Once sync is running, access your HQ at [hq.{company}ai.com](https://hq.{company}ai.com):

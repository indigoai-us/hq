...
- [ ] Register `@{company}ai` npm org (or verify access)
...
- [ ] Verify `{company}ai-us/hq` repo is public
...
npm view @{company}ai/hq-cli   # Shows package info
npm view @{company}ai/hq-cloud # Shows package info
```
...
1. **Register npm org** — `npm org create {company}ai` or verify existing

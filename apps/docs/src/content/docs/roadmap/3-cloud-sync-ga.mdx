...
Users can run `hq sync init` → `hq sync start` and have their HQ files accessible from any device via the PWA at example.com.
...
2. Browser opens to example.com/auth
...
2. **Configure domain** — DNS + SSL for example.com

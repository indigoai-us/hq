...
Users can run `hq sync init` → `hq sync start` and have their HQ files accessible from any device via the PWA at hq.{company}ai.com.

## Prerequisites

### AWS Account

- [ ] AWS account under {Product}AI organization
...
- [ ] `hq.{company}ai.com` DNS configured
...
2. Browser opens to hq.{company}ai.com/auth
...
1. Navigate to hq.{company}ai.com
...
2. **Configure domain** — DNS + SSL for hq.{company}ai.com
...
- [ ] PWA loads at hq.{company}ai.com

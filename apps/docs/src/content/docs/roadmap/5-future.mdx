...
**Idea**: `brew install {company}ai/tap/create-hq` as an alternative to npx.

**Why**: Faster install, no Node.js dependency for the installer itself, native feel on macOS.

**How**: Create a Homebrew tap repo (`{company}ai-us/homebrew-tap`) with a formula that downloads the compiled binary. Use `pkg` or `bun compile` to create a standalone executable from the TypeScript source.
...
plugins:
  slack-notify:
    repo: https://github.com/{company}ai-us/plugin-slack
    config:
...
Have an idea? Open an issue on [GitHub](https://github.com/{company}ai-us/hq/issues) with:

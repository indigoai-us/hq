...
1. **Target location** — if multiple valid options exist:
   ...
   - Existing shared base: `knowledge/{name}/`
...
2. If topic fits an existing shared base → `knowledge/{base}/{slug}.md`
3. If topic is general GHQ/engineering → `knowledge/ghq-core/{slug}.md`
...
Knowledge files live in the GHQ repo. Commit the new/updated file:

```bash
git add "{path to knowledge file}"
git commit -m "knowledge: {short topic summary}"
```

If git operations fail (nothing to commit), continue — the file is still written.
...
- **Ontology compliance** — all frontmatter must match ontology (type, domain, status, tags, relates_to)
...
Creates: `knowledge/ghq-core/supabase-rls-patterns.md`
Type: `guide`
Domain: `[engineering]`
...
Creates: `knowledge/ghq-core/real-time-architecture-decisions.md`
Type: `reference`

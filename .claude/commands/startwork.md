...
### 0. Check for missed curiosity cycles

Read `workspace/curiosity/cron-state.yaml` (if it exists). Check if a cycle is overdue:

- If `last_run` is `never` or if the time between `last_run` and now exceeds the `frequency` (default: daily = 24 hours), a cycle is missed.
- If `status` is `error`, also flag it.

If a cycle is missed, include a note in the orientation block (Step 3):

```
Curiosity cycle: overdue (last ran {last_run or "never"})
  Run /curiosity-cycle to update knowledge
```

Do NOT run the cycle automatically — just surface the suggestion. The user decides.

If cron-state.yaml does not exist, skip this check silently.

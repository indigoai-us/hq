...
If merge:
1. Commit any uncommitted loop state on main first: `git add loops/ && git commit -m "loop: complete {task-id}"`
2. Merge: `git merge {worktree-branch}` (resolve conflicts if state.jsonl diverged -- keep all entries from both sides)
3. Clean up worktree: `git worktree remove --force {worktree-path} && git branch -d {worktree-branch}`
...
- **Always commit and push** -- after merging a worktree or completing the loop, always commit any dirty state AND push to remote. Never leave changes local-only.

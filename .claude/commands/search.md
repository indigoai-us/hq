...
- `-c` — collection name (e.g. `hq`, `myproject`). Default: all collections
...
1. **cwd**: If inside `companies/{name}/` or `repos/` matching a company repo per `companies/manifest.yaml` → use that company's collection
...
Available company collections: configured in your qmd setup

Also: `hq` (all HQ), plus any repo collections you've added
...
Results:
  1. [0.92] hq: knowledge/Ralph/02-core-concepts.md
     "Ralph methodology emphasizes small loops with human checkpoints..."

  2. [0.84] myproject: libs/core/src/auth/middleware.ts
     "export function authMiddleware..."

  3. [0.71] hq: workers/dev-team/architect/skills/design-review.md
...
/search auth middleware -c myproject             # Search myproject codebase only
/search "webhook handler" -c myproject --mode vsearch  # Semantic search in myproject
...
/search "brand guidelines" -c company1           # Search company1 knowledge only
/search "metrics" -c company2                    # Search company2 knowledge only
# If cwd is companies/company1/:
/search "case study"                             # Auto-detects → -c company1
```
...
- Use `-c` to scope to a collection: `hq`, or any repo/company collection you've configured

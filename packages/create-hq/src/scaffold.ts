...
export async function scaffold(
  ...
  // 3. Git init
  if (hasGit()) {
    const { committed } = initGit(targetDir);
    if (committed) {
      success("Initialized git repository");
    } else {
      warn("Git initialized but initial commit failed (pre-commit hook blocked it). Run: cd " + displayDir + " && git add -A && git commit -m 'Initial setup'");
    }
  } else {
